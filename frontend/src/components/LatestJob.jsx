...
function LatestJob() {
  const { allJobs, searchJobs, searchedQuery } = useSelector(
    (state) => state.job,
  );
  const navigate = useNavigate();

  const displayedJobs = searchedQuery ? searchJobs : allJobs;

  return (
    <div className="max-w-7xl mx-auto mt-8 mb-24 px-6 relative z-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {!displayedJobs || displayedJobs.length === 0 ? (
            ...
            {!searchedQuery && (
              <>
                <p className="text-slate-400 mb-8 text-center max-w-md">
                  Unlock exclusive access to the latest job opportunities
          ...
          displayedJobs?.slice(0, 6)?.map((job, idx) => (
            <div
              key={job._id}
